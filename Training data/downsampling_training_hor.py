#Script to downsample finegrid data to coarsegrid, which is needed for the generation of the training data for the NN. This is here only done for the horizontal directions.
...
def downsample(finegrid, coarsegrid, variable_name, bool_edge_gridcell = (False, False, False), periodic_bc = (False, True, True), zero_w_topbottom = True):
    ...
    for ycor_c_middle in ycor_c:
        ...
        for xcor_c_middle in xcor_c:
            ...
            for k in range(var_finezyx.shape[0]):
                var_c[k,iyc,ixc] = np.sum(np.multiply(weights, var_finezyx[k,:,:]))
